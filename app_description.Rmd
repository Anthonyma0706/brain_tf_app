...
- make the ggplot image from download smaller
- lower the height of
- debug the cytoscape network, remove the "stop showing"
...
- input: a transcription factor, chosen from (3. Active TF data) vector TF_active
- output: 
  1. A heatmap that, given a specific TF, we filter the activity_cluster datasets with this TF then display a heatmap that shows the level of activity featured by NES(score). 
  You will see a gradient of color which makes a great straightforwad visualization.  
